...
def ticTacToe():
    ...
    while(won == False):
        ...
        while(invalidMove):
            ...
            if (tempMoves[spaceToMove] != '='):
                print("Can't move here try again")
            else:
                tempMoves[spaceToMove] = lastMove
                invalidMove = False

        #check for winner
        if(tempMoves[0] == tempMoves[1] == tempMoves[2]):
            won = True
        elif(tempMoves[3] == tempMoves[4] == tempMoves[5]):
            won = True
        elif(tempMoves[6] == tempMoves[7] == tempMoves[8]):
            won = True
        elif(tempMoves[0] == tempMoves[4] == tempMoves[8]):
            won = True
        elif(tempMoves[2] == tempMoves[4] == tempMoves[6]):
            won = True
        elif(tempMoves[0] == tempMoves[3] == tempMoves[6]):
            won = True
        elif(tempMoves[1] == tempMoves[4] == tempMoves[7]):
            won = True
        elif(tempMoves[2] == tempMoves[5] == tempMoves[8]):
            won = True

        draw = True
        for item in tempMoves:
            if(item == '='):
                draw = False

        if(draw and won == False):
            won = True
